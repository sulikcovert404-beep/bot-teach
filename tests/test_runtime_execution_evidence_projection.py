...
def test_trace_and_reference_binding():
 _,r=projection(); bad=build_evidence_projection(projection_id="p",execution_reference=ref("exec"),result_reference=ref("exec",digest=r.result_digest),evidence_type="validation",evidence_payload_reference=ref("payload",status=__import__('app.services.runtime_admission_bundle',fromlist=['ReferenceStatus']).ReferenceStatus.INVALID),validity_status=EvidenceValidityStatus.UNKNOWN,trace_reference=ref("trace")); assert not validate_evidence_projection(bad)
def test_deterministic_unicode_and_secret_rejection():
 a,_=projection(); b,_=projection(); assert a.canonical_bytes()==b.canonical_bytes() and a.projection_digest==b.projection_digest
