...
def att(status=EvidenceValidityStatus.VALID):
 r,_,_b=result(ExecutionResultStatus.SUCCEEDED); p=build_evidence_projection(projection_id="p",execution_reference=ref("exec"),result_reference=ref("exec",digest=r.result_digest),evidence_type="validation",evidence_payload_reference=ref("payload"),validity_status=status,trace_reference=ref("trace")); a=build_attestation(attestation_id="a",execution_reference=ref("exec"),result_reference=ref("exec",digest=r.result_digest),projection_references=(ref("p",digest=p.projection_digest),),boundary_reference=ref("boundary"),trace_reference=ref("trace"),validity_period_reference=ref("period")); return a,r,p
...
def test_deterministic_unicode_and_secret_rejection():
 a,_,_=att(); b,_,_=att(); assert a.canonical_bytes()==b.canonical_bytes() and a.attestation_digest==b.attestation_digest
