...
class ReleaseAuditOutcome(StrEnum):
...
class StagingActivationGovernanceReleaseReadinessFinalAudit:
 audit_id:str
 release_certification_bundle_reference:ReferenceToken
 ...
 def payload(self)->dict[str,Any]:
  names=("release_certification_bundle_reference","certification_bundle_reference","final_audit_reference","baseline_freeze_reference","freeze_assurance_reference","activation_control_package_reference","activation_decision_reference","trace_reference")
  out={"audit_id":self.audit_id,"release_execution":"PROHIBITED","deployment":"PROHIBITED","staging_activation":"PROHIBITED","runtime_activation":"PROHIBITED","runtime_admission":"PROHIBITED","execution":False,"audit_findings":self.audit_findings,"boundary_assertions":self.boundary_assertions}
...
def evaluate_release_audit(r:StagingActivationGovernanceReleaseReadinessFinalAudit)->ReleaseAuditOutcome:
 refs=[getattr(r,n) for n in ("release_certification_bundle_reference","certification_bundle_reference","final_audit_reference","baseline_freeze_reference","freeze_assurance_reference","activation_control_package_reference","activation_decision_reference","trace_reference")]
 if not r.digest_matches() or any(x.status is ReferenceStatus.BLOCKED for x in refs): return ReleaseAuditOutcome.RELEASE_BLOCKED
 if any(x.status is ReferenceStatus.INVALID for x in refs): return ReleaseAuditOutcome.RELEASE_NOT_READY
 if any(x.status is ReferenceStatus.REQUIRES_REVIEW for x in refs): return ReleaseAuditOutcome.UNKNOWN
 if r.audit_findings or not r.boundary_assertions or r.boundary_assertions.get("execution") is not False: return ReleaseAuditOutcome.RELEASE_NOT_READY
 if r.boundary_assertions.get("warnings"): return ReleaseAuditOutcome.RELEASE_READY_CONFIRMED_WITH_WARNINGS
 return ReleaseAuditOutcome.RELEASE_READY_CONFIRMED
