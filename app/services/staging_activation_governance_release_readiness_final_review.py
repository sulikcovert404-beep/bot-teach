...
class StagingActivationGovernanceReleaseReadinessFinalReview:
 review_id:str
 release_readiness_package_reference:ReferenceToken
 ...
 def payload(self)->dict[str,Any]:
  names=("release_readiness_package_reference","certification_bundle_reference","final_audit_reference","baseline_freeze_reference","freeze_assurance_reference","activation_control_package_reference","activation_decision_reference","trace_reference")
  out={"review_id":self.review_id,"release_execution":"PROHIBITED","deployment":"PROHIBITED","staging_activation":"PROHIBITED","runtime_activation":"PROHIBITED","runtime_admission":"PROHIBITED","execution":False,"review_findings":self.review_findings,"boundary_assertions":self.boundary_assertions}
...
def evaluate_final_review(r:StagingActivationGovernanceReleaseReadinessFinalReview)->FinalReviewOutcome:
 refs=[getattr(r,n) for n in ("release_readiness_package_reference","certification_bundle_reference","final_audit_reference","baseline_freeze_reference","freeze_assurance_reference","activation_control_package_reference","activation_decision_reference","trace_reference")]
 if not r.digest_matches() or any(x.status is ReferenceStatus.BLOCKED for x in refs): return FinalReviewOutcome.RELEASE_BLOCKED
 if any(x.status is ReferenceStatus.INVALID for x in refs): return FinalReviewOutcome.RELEASE_NOT_READY
 if any(x.status is ReferenceStatus.REQUIRES_REVIEW for x in refs): return FinalReviewOutcome.UNKNOWN
 if r.review_findings or not r.boundary_assertions or r.boundary_assertions.get("execution") is not False: return FinalReviewOutcome.RELEASE_NOT_READY
 if r.boundary_assertions.get("warnings"): return FinalReviewOutcome.RELEASE_READY_CONFIRMED_WITH_WARNINGS
 return FinalReviewOutcome.RELEASE_READY_CONFIRMED
