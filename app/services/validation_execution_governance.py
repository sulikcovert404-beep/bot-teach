...
class ValidationExecutionRequest:
    ...
    def canonical_bytes(self): return json.dumps(self.as_dict(), ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    def computed_digest(self): return hashlib.sha256(self.canonical_bytes()).hexdigest()
...
class GovernanceDecision:
    ...
    def canonical_bytes(self): return json.dumps(self.__dict__ if False else {"decision_reference": self.decision_reference, "evidence_reference": self.evidence_reference, "outcome": self.outcome.value, "reason": self.reason, "request_reference": self.request_reference}, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")

def evaluate_request(request, *, approved_plan, evidence_available, authorized, granted_capabilities=(), digest_valid=None, request_reference="request", decision_reference="decision", evidence_reference=None):
