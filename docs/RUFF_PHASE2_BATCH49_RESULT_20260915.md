...
Change: Removed only the two unused deprecated typing imports. `Any` and `Optional` remain unchanged. Endpoint definitions, route paths/methods, request/response models, dependencies/authorization, status codes, response shape, commercial-readiness metrics, serialization, and runtime behavior are unchanged.

Validation:
- Targeted Ruff (`UP006,UP035`): PASS (0 findings)
- py_compile: PASS
- Focused commercial-readiness/API tests: NO MATCHING TEST FILES
